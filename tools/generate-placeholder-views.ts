...
for(const id of ids){
 ...
 const edges=[{id:'north',color:'#bf5a54',shape:rectangle(0,0,120,7)},{id:'east',color:'#d2a94f',shape:rectangle(113,0,7,90)},{id:'south',color:'#4c9b78',shape:rectangle(0,83,120,7)},{id:'west',color:'#557fb0',shape:rectangle(0,0,7,90)}];const roads=edges.map(edge=>`<polygon data-edge="${edge.id}" points="${points(edge.shape)}" fill="${edge.color}"/>`).join('');
