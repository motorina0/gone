...
for(const id of ids){
 ...
 const structures=environment.landmarks.map(landmark=>{
  ...
  const walls=[0,1,2,3].map(index=>`<polygon points="${points([base[index]!,base[(index+1)%4]!,top[(index+1)%4]!,top[index]!])}" fill="${landmark.type==='building'||landmark.type==='tower'?'url(#cathedral-wall)':index%2?'#444945':'#505550'}" stroke="#9d9b8e"/>`).join('');
  const roof=landmark.type==='building'?`<polygon points="${points([top[0]!,top[1]!,{x:landmark.x+landmark.width/2,y:landmark.y+landmark.height/2,elevation:height+5},top[3]!])}" fill="url(#cathedral-roof)"/><polygon points="${points([top[1]!,top[2]!,top[3]!,{x:landmark.x+landmark.width/2,y:landmark.y+landmark.height/2,elevation:height+5}])}" fill="url(#cathedral-roof)"/>`:`<polygon points="${points(top)}" fill="${landmark.color}" stroke="#d0c5aa"/>`;
 ...
 const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="960" height="640" viewBox="0 0 960 640"><defs><pattern id="cathedral-wall" width="160" height="160" patternUnits="userSpaceOnUse"><image href="../textures/cathedral-wall.png" width="160" height="160" preserveAspectRatio="xMidYMid slice"/></pattern><pattern id="cathedral-roof" width="180" height="180" patternUnits="userSpaceOnUse"><image href="../textures/cathedral-roof.png" width="180" height="180" preserveAspectRatio="xMidYMid slice"/></pattern><linearGradient id="ground" x2="0" y2="1"><stop stop-color="#263b3b"/><stop offset="1" stop-color="#162728"/></linearGradient><style>.label{fill:#e2d7bd;font:11px system-ui;text-anchor:middle;paint-order:stroke;stroke:#101a1b;stroke-width:3px}.grid path{fill:none;stroke:#738784;stroke-width:1;opacity:.25}.roads polygon{fill:#65706d;opacity:.6}</style></defs><rect width="960" height="640" fill="#0d171a"/>${floor}<g class="grid">${grid.join('')}</g><g class="roads">${roads}</g><g class="orientation-margins">${marginLines}</g>${structures}${trees}${furniture}<text x="24" y="618" fill="#9eaaa4" font-family="system-ui" font-size="13">Original schematic placeholder · ${projection.name}</text></svg>`;
