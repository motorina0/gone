...
for vid in ids:
 ...
 ground=pts(rect(0,0,600,180))
 rails=''.join(f'<polyline points="{pts([(0,y,0),(600,y,0)])}"/>' for y in [51,57,63,97,103,109])
 sleepers=''.join(f'<polyline points="{pts([(x,48,0),(x,66,0)])}"/><polyline points="{pts([(x,94,0),(x,112,0)])}"/>' for x in range(0,601,8))
 buildings=[]; occ=[]; details=[]
 height_by={'station':14,'tower':28,'warehouse':11,'office':8,'utility':6,'bridge':7,'platform':1.2,'yard':0}
