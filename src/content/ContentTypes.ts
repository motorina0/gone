...
export interface Manifest{id:string;name:string;world:string;environment:string;mission:string;navigation:{walkable:string;blockers:string;visionBlockers:string};entities:string[];patrols:string[];interactions:Record<string,string>;projections:string[];views:string[];occlusion:string[]}
export interface LoadedContent{baseUrl:URL;manifest:Manifest;world:WorldResource;mission:MissionResource;entities:EntityResource[];patrols:PatrolResource[];blockers:Rect[];visionBlockers:Rect[];projections:ProjectionResource[];views:string[];occlusion:string[]}
