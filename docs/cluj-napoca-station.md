...
The field operative uses original eight-direction Blender art refined from `art/agent/references/gone-operative-turnaround.png`: dark weatherproof coat, charcoal knitwear and trousers, worn brown boots, and a practical satchel. Every location declares a canonical visual height of `1.8 m`. The compact atlas records a measured 137-pixel visible idle silhouette. Across all 72 high-resolution frames, the tallest pose is 1119 pixels and the combined non-transparent animation envelope spans rows 47–1177; both values are explicit manifest data. Runtime scale is `1.8 × projection scale ÷ visible pixels`, while close camera placement centers the full animation envelope so raised running poses cannot clip.
